...
|-------|-------------|---------------|-------------|----------|
| <a id="authenticatable"></a>**Authenticatable** | Laravel core authentication: `getAuthIdentifier()`, `getAuthPassword()`, `getRememberToken()` / `setRememberToken()`, and session-based login helpers. Framework trait — no owning module. | _(framework)_ | `Illuminate\Auth\Authenticatable` | _(framework; no deep doc)_ |
| <a id="authorizable"></a>**Authorizable** | Laravel Gate authorization: `can()` / `cannot()` / `canAny()` checks proxied through the `Gate` facade. Framework trait — no owning module. | _(framework)_ | `Illuminate\Foundation\Auth\Access\Authorizable` | _(framework; no deep doc)_ |
| <a id="canresetpassword"></a>**CanResetPassword** | Password-reset email dispatch via `sendPasswordResetNotification()`. Framework trait — no owning module. | _(framework)_ | `Illuminate\Auth\Passwords\CanResetPassword` | _(framework; no deep doc)_ |
...
| <a id="hasapitokens"></a>**HasApiTokens** | Laravel Sanctum: issues and manages API bearer tokens; `tokens()` relationship, `createToken()`, `tokenCan()`. Package trait — no owning module. | _(package: sanctum)_ | `Laravel\Sanctum\HasApiTokens` | _(package; no deep doc)_ |
...
| <a id="mustverifyemail"></a>**MustVerifyEmail** | Laravel email verification: `hasVerifiedEmail()`, `markEmailAsVerified()`, `sendEmailVerificationNotification()`. Framework trait — no owning module. | _(framework)_ | `Illuminate\Auth\MustVerifyEmail` | _(framework; no deep doc)_ |
| <a id="notifiable"></a>**Notifiable** | Laravel notifications: `notify()` / `notifyNow()`, `notifications()` morphMany relationship, `routeNotificationFor()`. Framework trait — no owning module. | _(framework)_ | `Illuminate\Notifications\Notifiable` | _(framework; no deep doc)_ |
...
| <a id="hastransactions"></a>**HasTransactions** | Polymorphic `transactions()` and `payments()` morphMany relationships (`transactionable` morph); `getTransactionsOfType()` helper. | Common | `modules/Common/Traits/HasTransactions.php` | _(deep doc pending)_ |
| <a id="hastransactionservice"></a>**HasTransactionService** | Access to `TransactionService` via factory: `createCharge()`, `createCredit()`, `createFinancingTransfer()`, `createInterest()`, and related helpers. | Common | `modules/Common/Traits/HasTransactionService.php` | _(deep doc pending)_ |
...
| <a id="twofactorauthenticatable"></a>**TwoFactorAuthenticatable** | Laravel Fortify two-factor authentication: `two_factor_secret` / `two_factor_recovery_codes` storage, `hasEnabledTwoFactorAuthentication()`, `twoFactorQrCodeSvg()`. Package trait — no owning module. | _(package: fortify)_ | `Laravel\Fortify\TwoFactorAuthenticatable` | _(package; no deep doc)_ |
