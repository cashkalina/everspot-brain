...
4. **Observer registrations** — grep for this model's `::observe(` registration. Observers are registered in **two** places in Everspot, and **both** must be searched:
   - `app/Providers/EventServiceProvider.php` (the global `$observers` map / boot registrations), and
   - the model's **own module service provider** — `modules/<Module>/Providers/<Module>ServiceProvider.php` — where module-scoped observers are registered in `boot()` (e.g. `Customer::observe(CustomerObserver::class)` in `CustomerServiceProvider`).

   Search both `app/Providers/*ServiceProvider.php` and `modules/*/Providers/*ServiceProvider.php` for the model's class name followed by `::observe(`. If registered, include the registering provider **and** the observer class file in `source_paths`. (Note: a module observer may itself dispatch events — e.g. `CustomerObserver` dispatches `CustomerUpdated` on `updated` — which is documented under the model's **Observers** section, not **Events**.)
